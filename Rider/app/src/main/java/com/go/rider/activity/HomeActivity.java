...
import com.go.rider.remote.NotificationHelper;
...
import com.go.rider.util.map.GpsUtils;
import com.go.rider.util.map.LatLngInterpolator;
import com.go.rider.util.map.MapDirection;
import com.go.rider.util.map.MarkerAnimation;
...
public class HomeActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    private void checkRiderToken(){
        FirebaseInstanceId.getInstance().getInstanceId().addOnCompleteListener(task -> {
            ...
            String currentToken = Objects.requireNonNull(task.getResult()).getToken();
    ...
    private void sentTripRequest(List<String> driverTokens){
        firebaseHelper.setData(FirebaseDatabase.getInstance().getReference(FirebaseHelper.TRIP_REQUESTS_TABLE).child(this.tripInfo.getTripId()).setValue(this.tripInfo),
                new FirebaseHelper.OnFirebaseUpdateListener() {
                    @Override
                    public void onSuccess() {
                        if(driverTokens.size() != 0) {
                            String data = new Gson().toJson(HomeActivity.this.tripInfo);
                            new NotificationHelper(driverTokens, "Trip Request",
